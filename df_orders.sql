...
select top 10 product_id,sum(sale) as sales from df_orders
group by product_id 
order by sales desc

---find top 5 selling products in each region
with cte as(
select region,product_id,sum(sale)as sales
from df_orders
group by region,product_id)

select * from(select * ,row_number() over(partition by region
order by sales desc) as rn  from cte) a where rn<=5 

---find month over month grow comparison for 2022 & 2023 sales
with cte as(
select year(order_date)as order_year,
month(order_date) as order_month,sum(sale) as sales
